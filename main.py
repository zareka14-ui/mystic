...
PORT = int(os.getenv("PORT", 8080)) # Render назначает порт сам
...
async def numerology_process(message: types.Message, state: FSMContext):
    ...
    date_obj = parse_date(message.text)
    # Если текст "ОК", берем сохраненную
...
async def handle(request):
    """Обработка запроса от Cron или браузера (чтобы Render не убил процесс)"""
...
async def main():
    ...
    app = web.Application()
    app.router.add_get('/', handle) # Роут для пинга
...
if __name__ == "__main__":
    try:
        ...
        print("Bot stopped.")import asyncio
import logging
import os
import sys
import json
